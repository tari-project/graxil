...
use sha3x_miner::core::{sha3x::*, calculate_difficulty};
...
fn test_sha3x_performance_baseline() {
    ...
    for (name, header) in test_cases {
        ...
        println!("  {} hashes in {:.3}s = {:.2} H/s", 
            hash_count, duration.as_secs_f64(), hashrate);
    }
...
fn test_sha3x_correctness() {
    ...
    assert_ne!(hash_a, hash_b, "Different nonces should produce different hashes");
    ...
    assert_ne!(hash_orig, hash_alt, "Different headers should produce different hashes");
    ...
    assert_eq!(manual_result, function_result, "Function should match manual implementation");
...
fn test_difficulty_calculation_performance() {
    ...
    println!("  {} difficulty calculations in {:.3}s = {:.2} calc/s", 
        iterations, duration.as_secs_f64(), calc_rate);
    println!("  Average difficulty: {:.2}", difficulty_sum as f64 / iterations as f64);
    ...
    assert!(calc_rate > 100000.0, "Difficulty calculation should be very fast");
}
...
fn test_hash_distribution_quality() {
    ...
    println!("📊 Analyzing hash distribution with {} samples...", sample_size);
    ...
    println!("  First byte zero count: {} ({:.2}%)", 
    ...
    println!("  Maximum relative deviation: {:.1}%", max_relative_deviation);
    ...
    println!("  Average bit flips per input bit change: {:.1}", avg_bit_flips);
...
fn test_performance_consistency() {
    ...
    println!("📊 Running {} performance tests of {} hashes each...", num_runs, iterations_per_run);
    ...
    println!("  Range: {:.2} H/s ({:.1}%)", max_rate - min_rate, ((max_rate - min_rate) / mean) * 100.0);
    println!("  Coefficient of variation: {:.1}%", coefficient_of_variation);
    ...
    assert!(coefficient_of_variation < 25.0, "Performance too inconsistent");
}
