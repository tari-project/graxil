...
fn test_batch_vs_single_performance() {
    ...
    let test_header = vec![
        ...
        0x88, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44,
        0x33, 0x22,
    ];
    ...
    println!("📈 RESULTS:");
    println!(
        "  Single Hash: {:.2} H/s ({:.3}s for {} hashes)",
        single_hashrate,
        single_duration.as_secs_f64(),
        single_hashes
    );
    println!(
        "  Batch Hash:  {:.2} H/s ({:.3}s for {} hashes)",
        batch_hashrate,
    ...
    assert!(
        improvement > -10.0,
        "Batch processing significantly slower than expected"
    );
}
...
fn test_different_batch_sizes() {
    ...
    // Basic sanity check
    assert!(
        results.len() == batch_sizes.len(),
        "All batch sizes should be tested"
    );
}
...
fn test_batch_memory_efficiency() {
    ...
    println!(
        "  Tested {} batch operations ({} individual hashes)",
        iterations,
        iterations * 4
    );
    ...
    assert_eq!(
        mismatches, 0,
        "Batch results should match single hash results exactly"
    );
...
fn test_batch_performance_under_load() {
    ...
    let test_header = vec![
        0x5a, 0x6b, 0x7c, 0x8d, 0x9e, 0xaf, 0xb0, 0xc1, 0xd2, 0xe3, 0xf4, 0x05, 0x16, 0x27, 0x38,
        0x49, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88, 0x99,
    ];

    let high_iterations = 50000; // Higher load test
    println!(
        "📊 Running high-load test ({} iterations)...",
        high_iterations
    );
    ...
    println!(
        "  Per-batch time: {:.3}ms",
        (duration.as_secs_f64() * 1000.0) / (high_iterations as f64 / 4.0)
    );
