...
pub async fn start_web_server(stats: Arc<MinerStats>) {
    ...
    let listener = match tokio::net::TcpListener::bind("0.0.0.0:8080").await {
        ...
        Err(e) => {
            error!("❌ Failed to bind web server to port 8080: {}", e);
            error!("💡 Make sure port 8080 is not already in use");
            return;
        }
    };

    info!("🌐 Web dashboard available at: http://localhost:8080");
    info!("📊 Real-time charts at: http://localhost:8080 (Live Charts tab)");
...
async fn handle_socket(mut socket: WebSocket, stats: Arc<MinerStats>) {
    ...
    loop {
        ...
        let json = match serde_json::to_string(&data) {
            Ok(json) => json,
            Err(e) => {
                error!("❌ Failed to serialize mining data: {}", e);
                break;
            }
        };
